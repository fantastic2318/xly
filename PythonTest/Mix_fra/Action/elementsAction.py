...
from PythonTest.Mix_fra.Util.find_elements import locate_element, locate_elements
...
def click_list(location_type, location_express, *args):
    """
    单击操作
    :param location_type:
    :param location_express:
    ...
    try:
        ...
        locate_elements(driver, location_type, location_express)[int(index)].click()
    except Exception as e:
        raise e
...
def input_content_list(location_type, location_express, value, *args):
    ...
    try:
        location_express, index = location_express.split(",")
        locate_elements(driver, location_type, location_express)[int(index)].send_keys(value)
    except Exception as e:
        raise e
...
#     close_browser('')
    click('xpath', "/html/body/header/nav/div/ul/li[2]/div[3]")
    time.sleep(10)
    refresh_page()
    click("class name", "nui-btn-text")
    input_content("id", "input_N", "张三")
    input_content_list('class name', "nui-ipt-input,4", "764904579@qq.com")
    click("class name", "nui-chk-text")
    input_content_list("class name", "nui-ipt-input,5", "18602495070")
    input_content('id', "input_DETAIL", "备注")
    click_list('class name', "nui-btn-text,6")
